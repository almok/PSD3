...
public class GameRules implements Initializable {
	@FXML
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		ArrayList<String[]> arr = PSDSingleton.getInstance().getGameRulesData();
		...
		saveButton.setOnAction(e -> {
			this.saveFunction("DataBase/Personal/GameRules.csv");
		});
	...
	private void saveFunction(String file) {
		ArrayList<String[]> gameRulesData = new ArrayList<>();
		...
		for (int i = 0; i < this.textFieldsArr.size(); i++) {
			...
			gameRulesData.add(data);
		}
		PSDSingleton.getInstance().setGameRulesData(gameRulesData);
		PSDSingleton.getInstance().saveGameRulesData();
	}
}
