...
public class AYNEmployeeLine {
	...
	private ArrayList<String> departments = new ArrayList<>(Arrays.asList("Sales", "Production\nPlanning",
			"Goods Receipt", "Floor Control", "Production", "Ledger", "Quality\nAssurance", "Finance", "Manager"));
	AYNEmployee emp;
	...
	public AYNEmployeeLine(AYNEmployee emp) {
		...
		skill.setOnAction(e -> {
			emp.setMultiSkilled(skill.isSelected());
		});

		// set button styles
		if (emp.getTime1() != 0 && emp.getTime2() != 0) {
			for (Button but : buttons) {
				if (Integer.parseInt(but.getText()) >= Math.min(emp.getTime1(), emp.getTime2())
						&& Integer.parseInt(but.getText()) <= Math.max(emp.getTime1(), emp.getTime2())) {
